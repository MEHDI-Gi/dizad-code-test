...
import { MaterialIcons, Ionicons, Foundation, MaterialCommunityIcons, Entypo, FontAwesome6 } from '@expo/vector-icons';
import { Skeleton } from 'moti/skeleton';
import VipBadge from '../components/elements/VipBadge';
import HeartBadge from '../components/elements/HeartBadge';
import Animated, {
  useSharedValue,
  withTiming,
  useAnimatedStyle,
...
const QuizScreen = () => {
  ...
  const [isAnswersSelected, setIsAnswersSelected] = useState(false)
  const [isPointsAvailable, setIsPointsAvailable] = useState(false)
  const [isAnsBtnAct, setIsAnsBtnAct] = useState(null);
  const [buttonsStates, setButtonsStates] = useState([false, false, false]);
  const [checkedButtonIndex, setCheckedButtonIndex] = useState(null);
  const [trueAnswer, setTrueAnswer] = useState(null);
  const [showCorrectAnswerIndex, setShowCorrectAnswerIndex] = useState(null);
  const [isPointHelpPressed, setIsPointHelpPressed] = useState(null);

  const [answerSelectPosition, setAnswerSelectPosition] = useState(0);
  const [helpSnackPosition, setHelpSnackPosition] = useState(0);
  ...
  function TimeEndNextExit() {
    ...
    setIsPointHelpPressed(null);
    setIsAnsBtnAct(null);
    setShowCorrectAnswerIndex(null);
    setIsChecked(false);
    updateQuestIndex(`ct${dataLevelIndex}`, questIndices[`ct${dataLevelIndex}`] + 1);
  ...
  const handleAnswerSelect = (answers, index) => {
    setIsChecked(true)
    setIsAnsBtnAct(index);
  ...
  const checkAnswer = () => {
    if (selectedAnswer === '' || !isChecked) {
      ...
      setTrueAnswer(true);
      setGlobTrueAns((prev) => prev + 1);
      setIsButtonChecked(true);
      updateAnswerStats(`ct${dataLevelIndex}`, { correct: 1 });
      if (timerTimeout.current) {
        clearTimeout(timerTimeout.current);
        timerTimeout.current = null; // reset ref
        setHandleTimerBackground(true);
      }
    } else if (selectedAnswer !== rgtAns && isChecked) {
      ...
      setAnswersResult('إجابة خاطئة');
      setGlobFalseAns((prev) => prev + 1);
      console.log(`${livesHeart} from check answer`)
    }
  };
  ...
  const HelpPoints = () => {
    if (helpPoint >= 1) {
      ...
      const correctIndex = quizData[`ct${dataLevelIndex}`][`Q${questIndices[`ct${dataLevelIndex}`]}`]['Answers'].findIndex(
        (answer) => answer === quizData[`ct${dataLevelIndex}`][`Q${questIndices[`ct${dataLevelIndex}`]}`]['RightAnswer']
      );

      if (correctIndex !== -1) {
        setHelpPoint(prev => prev - 1)
      ...
      <View
        ...
        <PaperProvider>
          ...
          <View style={[styles.content]}>
            ...
            <View style={styles.questTextContainer}>
              <View style={{
                ...
                <Icon size={12} style={{ marginHorizontal: 3, }} color={colors.priText} source="pound" />
          ...
          <Animated.View
            ...
            <View style={{ backgroundColor: colors.primary, width: '100%', height: '100%', position: 'absolute' }}></View>
            <View style={{ width: '100%', padding: 20, alignItems: 'center', justifyContent: 'center', }}>
              <View style={{ backgroundColor: '#ce741321', borderRadius: 50, width: 45, height: 45, alignItems: 'center', justifyContent: 'center' }}>
                <MaterialCommunityIcons name={'timer-sand-complete'} color={'#ce7413'} size={25} />
              </View>
            </View>
            <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ color: 'white', fontFamily: 'Cairo_700Bold', fontSize: 20, }}>إنتهى الوقت !</Text>
            </View>
            <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center' }}>
              <Pressable
                android_ripple={{ color: colors.primary, borderless: false }}
                style={[
                  styles.nextButton, {
                    backgroundColor: '#a3601900',
                    borderColor: '#ce7413',
                    borderWidth: 2,
                    elevation: 0,
                  }]}
                onPress={NextButton}>
                <Text style={[styles.checkButtonText, { color: "#ce7413" }]}>Next</Text>
              </Pressable>
              <Pressable
                android_ripple={{ color: colors.primary, borderless: false }}
                style={[{
                ...
                onPress={TimeEndNextExit}>
                <Entypo name='home' color={colors.priText} size={16} />
                <Text style={[{ fontSize: 15, fontWeight: '400', color: "white" }]}>Back</Text>
              </Pressable>
            </View>
          </Animated.View>
