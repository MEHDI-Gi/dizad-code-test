...
export default function Apparence() {
    ...
    const apparenceList = [
        ...
        { label: 'Default', condition: 'default', icon: 'sunny', },
    ]
    const themeMap = {
        dark: { theme: THEME_DARK, colors: colorsList.darkColors },
        light: { theme: THEME_LIGHT, colors: colorsList.lightColors },
        light: { theme: THEME_DEFAULT, colors: colorsList.defaultColors },
    };
    const apparenceListPress = (item) => {
        ...
        setLoadingOptions({ icon: 'theme-light-dark' })
        setLoadScreen(true);
    };
    ...
    useEffect(() => {
        if (loadScreen) setTimeout(() => {
            setSnackOptions({ label: `${texts.themeChanged} ${currentTheme === 'dark' ? texts.dark : 'light' ? texts.light : 'Default'}`, icon: currentTheme === 'dark' ? 'moon-waning-crescent' : 'white-balance-sunny' })
    ...
    if (loadScreen) {
        return (
            <LoadingScreen
                mainIconSize={45}
                mainIconColor={currentTheme === 'dark' ? 'gray' : 'black'}
                indicatorSize={'small'}
            />
        )
    }
            ...
            <View style={{
                ...
                <View style={{
                    ...
                    <Pressable
                        ...
                        onPress={() => { setIsGradient(prev => !prev) }}>
                        <View style={{
                            width: 47,
                            height: 47,
                            alignItems: "center",
